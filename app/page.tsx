...
import { editVideo, Template } from "@/lib/ffmpeg";
...
// Extract a frame from video using canvas — no FFmpeg needed
async function extractFrameFromCanvas(file: File): Promise<string | null> {
...
export default function Home() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Use canvas-based frame extraction — works everywhere, no FFmpeg needed
      const frameBase64 = await extractFrameFromCanvas(file);
    ...
    <main className="app">
          ...
          <div className="two-col">
            <div className="left-panel">
              ...
              <div className="action-row">
                <button className="btn-primary" onClick={handleEdit} disabled={isEditing || isAnalyzing}>
                  {isEditing ? `Processing… ${progress}%` : "▶ Apply & Export"}
                </button>
                <button className="btn-secondary" onClick={handleAnalyze} disabled={isAnalyzing || isEditing}>
            ...
            <div className="right-panel">
              ...
              {analyzeError && (
                <div className="error-card">
                  <p>⚠ {analyzeError}</p>
                  <button className="btn-secondary" style={{marginTop:"12px", fontSize:"13px", padding:"10px"}} onClick={handleAnalyze}>
                    Retry
                  </button>
                </div>
              )}
                ...
                <div className="analysis-results">
                  <div className="vibe-card">
                    <div className="vibe-top">
                      <span className="energy-badge" style={{ background: energyColor[analysis.energy] ?? "#888" }}>
                    ...
                    {analysis.songs?.map((song, i) => (
                      <div key={i} className="song-card">
                        ...
                        <button
                          ...
                          disabled={loadingMusic === i}
                        >
                          {loadingMusic === i ? "Searching Pixabay…" : `🎧 Find free music: "${song.pixabayKeyword}"`}
                        </button>

                        {activeSong === i && loadingMusic !== i && pixabayTracks.length > 0 && (
                          <div className="pixabay-tracks">
                            {pixabayTracks.map((track) => (
                              <div key={track.id} className="track-row">
                                ...
                                <a href={track.pageUrl} target="_blank" rel="noopener noreferrer" className="download-link">
                                  ↗ Open
                                </a>
                              </div>
                            ))}
                        ...
                        {activeSong === i && loadingMusic !== i && pixabayTracks.length === 0 && (
                          <p className="no-tracks">No tracks found — try another keyword</p>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <ul className="tips-list">
                    {analysis.templateTips?.map((tip, i) => <li key={i}>{tip}</li>)}
                  </ul>

                  <h3 className="section-title">#️⃣ Hashtag Pack</h3>
                  <div className="hashtag-cloud">
                    {analysis.hashtags?.map((tag, i) => (
                      <span key={i} className={`hashtag ${copiedTag === tag ? "copied" : ""}`} onClick={() => copyTag(tag)} title="Click to copy">
        ...
        <section className="result-section">
          ...
          <div className="result-wrap">
            ...
            <div className="result-actions">
              <a href={outputUrl} download="reel.mp4" className="btn-primary">↓ Download Reel</a>
              <button className="btn-secondary" onClick={() => { setStep("edit"); setOutputUrl(null); }}>
                ← Try another template
              </button>
              {!analysis && (
                <button className="btn-secondary" onClick={() => { setStep("edit"); handleAnalyze(); }}>
                  ✦ AI Analyze this video
                </button>
              )}
              <button className="btn-ghost" onClick={() => { setStep("upload"); setFile(null); setVideoUrl(null); setAnalysis(null); setOutputUrl(null); }}>
                Upload new video
              </button>
            </div>
          </div>
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #080b0f; color: #e8eaf0; font-family: 'DM Sans', sans-serif; min-height: 100vh; }
        .app { max-width: 1200px; margin: 0 auto; padding: 24px 16px 80px; }

        .header { display: flex; align-items: baseline; gap: 16px; padding: 0 0 32px; border-bottom: 1px solid #1a1f2b; margin-bottom: 40px; }
        .logo { display: flex; align-items: center; gap: 10px; }
        .logo-icon { font-size: 28px; color: #ff6b35; }
        .logo-text { font-family: 'Bebas Neue', sans-serif; font-size: 32px; letter-spacing: 4px; color: #fff; }
        ...
        .drop-zone { width: 100%; max-width: 520px; border: 2px dashed #2a3040; border-radius: 20px; padding: 64px 32px; text-align: center; cursor: pointer; transition: all 0.2s; background: #0d1117; }
        ...
        .pill { padding: 8px 16px; border-radius: 100px; border: 1px solid #2a3040; font-size: 13px; color: #8892a4; background: #0d1117; }

        .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 32px; }
        @media (max-width: 800px) { .two-col { grid-template-columns: 1fr; } }
        .left-panel, .right-panel { display: flex; flex-direction: column; gap: 20px; }

        .video-preview-wrap { border-radius: 16px; overflow: hidden; background: #0d1117; border: 1px solid #1a1f2b; }
        .video-preview { width: 100%; max-height: 320px; object-fit: contain; display: block; }
        .file-name { font-size: 12px; color: #4a5568; }

        .section-title { font-size: 13px; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; color: #ff6b35; margin-top: 4px; }

        .template-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .template-card { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 16px 12px; border-radius: 14px; border: 1.5px solid #1a1f2b; background: #0d1117; cursor: pointer; transition: all 0.15s; text-align: center; }
        ...
        .btn-primary { flex: 1; padding: 14px; background: #ff6b35; color: #fff; border: none; border-radius: 12px; font-size: 14px; font-weight: 600; cursor: pointer; transition: opacity 0.2s; text-align: center; text-decoration: none; display: inline-block; }
        .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-primary:hover:not(:disabled) { opacity: 0.88; }
        .btn-secondary { flex: 1; padding: 14px; background: transparent; color: #ff6b35; border: 1.5px solid #ff6b35; border-radius: 12px; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
        .btn-secondary:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-secondary:hover:not(:disabled) { background: #1a0f08; }
        .btn-ghost { padding: 14px 20px; background: transparent; color: #4a5568; border: 1px solid #2a3040; border-radius: 12px; font-size: 14px; cursor: pointer; transition: all 0.2s; }
        .btn-ghost:hover { color: #e8eaf0; border-color: #4a5568; }

        .progress-bar { height: 4px; background: #1a1f2b; border-radius: 2px; overflow: hidden; }
        .progress-fill { height: 100%; background: #ff6b35; border-radius: 2px; transition: width 0.3s; }

        .error-msg { font-size: 13px; color: #ef5350; padding: 10px 14px; background: #1a0808; border-radius: 8px; border: 1px solid #3a1010; }
        .error-card { border: 1px solid #3a1010; background: #1a0808; border-radius: 16px; padding: 24px; color: #ef5350; font-size: 14px; }

        .analysis-placeholder { border: 1px dashed #2a3040; border-radius: 16px; padding: 32px; color: #4a5568; font-size: 14px; line-height: 1.8; }
        ...
        .spinner { width: 40px; height: 40px; border: 3px solid #1a1f2b; border-top-color: #ff6b35; border-radius: 50%; margin: 0 auto 16px; animation: spin 0.8s linear infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }

        .vibe-card { background: #0d1117; border: 1px solid #1a1f2b; border-radius: 16px; padding: 20px; }
        .vibe-top { display: flex; gap: 10px; align-items: center; margin-bottom: 12px; }
        .energy-badge { padding: 4px 10px; border-radius: 100px; font-size: 11px; font-weight: 700; color: #fff; letter-spacing: 1px; }
        ...
        .song-card { background: #0d1117; border: 1px solid #1a1f2b; border-radius: 14px; padding: 16px; }
        ...
        .bpm-tag { padding: 3px 8px; border-radius: 6px; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
        ...
        .pixabay-btn { font-size: 12px; color: #ff6b35; background: transparent; border: 1px solid #2a1a10; border-radius: 8px; padding: 8px 12px; cursor: pointer; transition: all 0.15s; width: 100%; text-align: left; }
        .pixabay-btn:hover { background: #1a0f08; }
        .pixabay-btn:disabled { opacity: 0.5; cursor: not-allowed; }

        .pixabay-tracks { margin-top: 10px; display: flex; flex-direction: column; gap: 8px; border-top: 1px solid #1a1f2b; padding-top: 10px; }
        ...
        .download-link { color: #ff6b35; font-size: 12px; font-weight: 600; text-decoration: none; flex-shrink: 0; padding: 5px 10px; border: 1px solid #2a1a10; border-radius: 6px; transition: background 0.15s; white-space: nowrap; }
        ...
        .tips-list li { font-size: 13px; color: #b0bac8; padding: 10px 14px; background: #0d1117; border-left: 3px solid #ff6b35; border-radius: 0 10px 10px 0; line-height: 1.5; }

        .hashtag-cloud { display: flex; flex-wrap: wrap; gap: 8px; }
        .hashtag { padding: 6px 12px; background: #0d1117; border: 1px solid #2a3040; border-radius: 8px; font-size: 12px; color: #64b5f6; cursor: pointer; transition: all 0.15s; user-select: none; }
        .hashtag:hover { background: #1a2030; border-color: #64b5f6; }
        .hashtag.copied { background: #0a2010; border-color: #4caf50; color: #4caf50; }
        ...
        .result-badge { display: inline-block; padding: 6px 16px; background: #1a0f08; border: 1px solid #ff6b35; color: #ff6b35; border-radius: 100px; font-size: 12px; font-weight: 600; letter-spacing: 2px; margin-bottom: 12px; }
        ...
        .reel-frame { width: 280px; border-radius: 20px; overflow: hidden; border: 1px solid #2a3040; flex-shrink: 0; box-shadow: 0 20px 60px rgba(255,107,53,0.15); }
        .output-video { width: 100%; display: block; }
        .result-actions { display: flex; flex-direction: column; gap: 12px; padding-top: 20px; min-width: 200px; }
      `}</style>
    </main>
  );
}
